...
public class DBSwing10 {
	...
	public static ArrayList<Profesor> consulta(String texto , String campoOrden, String orden) {
		...
		try {
			...
			if (campoOrden.length() > 0) {
				query += " ORDER BY " + campoOrden + " "+ orden;		//la onsulta sadra 
			}
			...
			while (rs.next()) {
				lista.add(new Profesor(rs.getString("profesor.nombre"),//la refernciamos en tabla y el campo
