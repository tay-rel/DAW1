...
public class DATABASE {
	...
	public static String singUp(String username, String password, String name, String nif, String email, String address,
		...
		try {
			...
			String query = plantilla.replaceAll("<<Username>>", username).replaceAll("<<Password>>", Utils.getMD5(password))
					.replaceAll("<<Nombre>>", name).replaceAll("<<NIF>>", nif).replaceAll("<<Email>>", email)
					.replaceAll("<<Direccion>>", address).replaceAll("<<Fecha de Nacimiento>>", Utils.formatDateSQL(birthdate));
	...
	public static User login(String username, String password) {
		...
		//Comprueba: System.out.println("SELECT * FROM users WHERE username='" + username + "' AND password='" + password + "';");
